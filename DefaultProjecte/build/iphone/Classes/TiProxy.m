...
//Rare exceptions to indicate a bug in the _defaultprojecte code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_defaultprojecte = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_defaultprojecte]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.defaultprojecte.module"
