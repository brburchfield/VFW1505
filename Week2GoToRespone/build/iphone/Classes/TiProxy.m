...
//Rare exceptions to indicate a bug in the _week2gotorespone code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_week2gotorespone = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_week2gotorespone]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.week2gotorespone.module"
