...
//Rare exceptions to indicate a bug in the _burchfield_beau_week3gototrainingreflection code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_burchfield_beau_week3gototrainingreflection = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_burchfield_beau_week3gototrainingreflection]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.burchfield_beau_week3gototrainingreflection.module"
