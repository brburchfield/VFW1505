...
NSString * Burchfield_Beau_Week3GoToTrainingReflection$ModuleRequireFormat = @"(function(exports){"
...
@implementation Burchfield_Beau_Week3GoToTrainingReflectionObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_burchfield_beau_week3gototrainingreflection);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_burchfield_beau_week3gototrainingreflection gc];
}
...
{
	// create Burchfield_Beau_Week3GoToTrainingReflection global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Burchfield_Beau_Week3GoToTrainingReflection" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_burchfield_beau_week3gototrainingreflection = [[Burchfield_Beau_Week3GoToTrainingReflectionObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_burchfield_beau_week3gototrainingreflection];
	
	NSString *_burchfield_beau_week3gototrainingreflectionNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _burchfield_beau_week3gototrainingreflectionNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_burchfield_beau_week3gototrainingreflection valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Burchfield_Beau_Week3GoToTrainingReflection$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.burchfield_beau_week3gototrainingreflection.kroll" 
