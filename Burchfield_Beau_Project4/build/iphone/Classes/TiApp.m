...
- (NSString*)userAgent
{
	if (userAgent==nil)
	{
		...
		NSString *kBurchfield_Beau_Project4UserAgentPrefix = [NSString stringWithFormat:@"%s%s%s %s%s","Appc","eler","ator","Tita","nium"];
		userAgent = [[NSString stringWithFormat:@"%@/%s (%@)",kBurchfield_Beau_Project4UserAgentPrefix,TI_VERSION_STR,currentDeviceInfo] retain];
	}
	return userAgent;
}
...
+(NSDictionary *)tiAppProperties
{
    ...
    if(props == nil) {
        ...
        if(jsonData) {
            ...
            // If we have no data...
            // This should never happen on a Burchfield_Beau_Project4 app using the node.js CLI
