...
NSString * Burchfield_Beau_Project3$ModuleRequireFormat = @"(function(exports){"
...
@implementation Burchfield_Beau_Project3Object
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_burchfield_beau_project3);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create Burchfield_Beau_Project3 global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Burchfield_Beau_Project3" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_burchfield_beau_project3 = [[Burchfield_Beau_Project3Object alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_burchfield_beau_project3];
	
	NSString *_burchfield_beau_project3NS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _burchfield_beau_project3NS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_burchfield_beau_project3 valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Burchfield_Beau_Project3$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.burchfield_beau_project3.kroll" 
